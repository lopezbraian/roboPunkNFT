...
const roboPunksNFTAddress = process.env.REACT_APP_CONTRACT_ADDRESS;

const MainMint = ({ accounts }) => {
  const [mintAmount, setMintAmount] = useState(1);
  const isConnected = Boolean(accounts[0]);
  const [totalMint, setTotaMint] = useState(0);
  ...
  const getTotalMint = useCallback(
